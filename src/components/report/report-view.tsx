...
function Section({
  ...
  return (
    <section data-testid={`section-${id}`} className="mt-10">
      <h3 className="mb-4 border-b border-[var(--ns-border)] pb-2 text-sm font-bold uppercase tracking-wider text-[var(--ns-fg-dim)]">
        {title}
      </h3>
      {children}
    </section>
...
export function ReportView({
  ...
  return (
    <article className="mx-auto max-w-4xl px-4 py-10 sm:px-6">
      <header className="border-b-2 border-[var(--ns-fg)] pb-6">
        <p className="font-instrument text-xs font-bold uppercase tracking-widest text-[var(--ns-accent)]">{dict.app.name}</p>
        <h1 className="mt-1 font-display text-2xl tracking-tight text-[var(--ns-fg)] sm:text-3xl">
      ...
      <footer className="mt-12 border-t border-[var(--ns-border)] pt-4 text-center text-[11px] text-[var(--ns-fg-dim)]">
      ...
      {showChrome && (
        <div className="no-print mt-6 flex flex-wrap items-center justify-center gap-3">
          ...
          <a href={`?print=1`} className="ns-btn-secondary">
            {t(dict, 'report.print')}
          </a>
        </div>
      )}
