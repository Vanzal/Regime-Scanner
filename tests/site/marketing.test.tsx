...
describe('marketing chrome copy', () => {
  it('preserves the live NexusScope headline and waitlist CTA', () => {
    ...
    expect(en.site.footer.operator_name).toBe('')
    expect(en.site.footer.company_name).toBe('NexusScope')
    expect(en.site.footer.contact_email).toBe('hello@nexusscopes.com')
    expect(en.site.legal.placeholder).toBe('The operator of NexusScope')
    expect(getDict('de').site.legal.placeholder).toBe('Der Betreiber von NexusScope')
    const copy = JSON.stringify(en) + JSON.stringify(getDict('de'))
    expect(copy).not.toMatch(/\bNoah\b/)
    expect(copy).not.toMatch(/Baumann/)
  })
  ...
  it('footer still lists imprint, privacy, contact and Switzerland / EU', () => {
    ...
    expect(html).toContain('Sole proprietorship (Switzerland)')
