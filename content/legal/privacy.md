...
- the **Federal Data Protection and Information Commissioner (FDPIC)**, Feldeggweg 1, 3003 Bern, Switzerland — [edoeb.admin.ch](https://www.edoeb.admin.ch/)
...
1. **Complete the sub-processor table in Section 6 with real vendors** and publish the live list. This is the single most common gap in SaaS privacy notices.
2. **Verify each vendor's transfer mechanism** — DPF certification status for US vendors changes; check the [DPF list](https://www.dataprivacyframework.gov/) rather than assuming.
3. **Confirm your retention periods match reality.** If your backups are retained 180 days, do not write 90.
4. **Art. 19 revFADP** requires informing data subjects at the point of collection, including the countries data is exported to — Section 7 covers this only if you fill in the actual destinations.
5. **Maintain a Record of Processing Activities** (Art. 30 GDPR / Art. 12 revFADP). Small businesses under 250 employees have a partial exemption, but it does not apply where processing is regular or involves sensitive data — a compliance-scanning product almost certainly processes regularly.
6. **Appoint the EU representative** (Art. 27 GDPR) and name them in Section 2.
7. **Build the rights workflow before you promise it** — a working export and delete function, plus an inbox that someone monitors, or the one-month deadline in Section 12 will be missed.
8. **Cookie banner must be configured to match Section 8** — reject as easy as accept, no pre-ticked boxes, no cookie set before consent.
9. **Have a Swiss/EU-qualified lawyer review the final text.**
